	...
	switch($mode) {
		...
		case "resetPassword":
								...
								submitHandler: function(form) {
									...
									auth.confirmPasswordReset('$actionCode', newPassword).then(function(resp) {
										...
										auth.signInWithEmailAndPassword(email, newPassword).then(function(resp) {
											...
											db.ref('users/' + firebase.auth().currentUser.uid).once('value').then(function(snapshot) {
												var form = $('<form></form>');
			...
			$form = "<form id='mgmt_form' method='post'>
					<div class='row'>
					<div class='col-xs-12 col-md-9 col-md-offset-1'><h5 style='margin-bottom:15px; font-weight:bold;'>User: <span></span></h5></div>
					...
					<input id='password' class='form-control' placeholder='Password' type='password' name='password' size='10' />
					</label>
					</div>
					</div>
					...
					<input id='password_confirm' class='form-control' placeholder='Confirm password' type='password' name='password_confirm' size='10' />
					</label>
					</div>						
					</div>
